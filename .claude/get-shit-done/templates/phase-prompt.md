...
**Verify upstream contracts when phase depends on previous phase output.**

When your phase integrates with output from a previous phase, add an explicit verification task BEFORE implementation tasks that checks the upstream component's actual output.

**When to add verification task:**
- Phase consumes data from previous phase (e.g., Phase 12 uses Phase 11's QueryEngine output)
- Implementing API client for another service's endpoints
- Building UI that renders data from backend API
- Processing files/data produced by earlier pipeline stage

**Pattern:**

```markdown
<task type="auto">
  <name>Task 1: Verify [UpstreamComponent] output contract</name>
  <files>N/A - verification only</files>
  <action>
    Before implementing [YourComponent], verify [UpstreamComponent] returns expected data structure:

    Run [UpstreamComponent] and inspect output:
    ```[language]
    result = upstream_component.method(...)
    print(result.keys())  # Verify schema
    print(result['expected_field'])  # Verify field exists
    ```

    Confirm presence of required fields:
    - [field1]: [expected type/format]
    - [field2]: [expected type/format]

    If contract differs from assumptions, document mismatch and adjust plan.
  </action>
  <verify>Upstream component returns all required fields</verify>
  <done>Contract verified, required fields present and correct format</done>
</task>
```

**Example from Phase 12 (would have prevented integration bug):**

```markdown
<task type="auto">
  <name>Task 1: Verify QueryEngine output contract</name>
  <files>N/A - verification only</files>
  <action>
    Before implementing AnswerGenerator, verify QueryEngine returns full chunk content:

    ```python
    from workers.retrieval import QueryEngine
    qe = QueryEngine(...)
    result = qe.query("test query", repo_id, top_k=1)

    # Verify schema
    chunk = result['results'][0]
    print("Available fields:", chunk.keys())

    # Verify full content field exists
    assert 'content' in chunk, "Missing 'content' field!"
    assert len(chunk['content']) > 200, "Only has content_preview!"
    ```

    Required fields for AnswerGenerator:
    - content: Full chunk text (not truncated preview)
    - file_path: Source file path
    - start_line, end_line: Line numbers
    - breadcrumb: Context path
  </action>
  <verify>QueryEngine returns 'content' field with full text</verify>
  <done>Contract verified, all required fields present</done>
</task>
```

**Why this matters:**
- Catches contract mismatches early (before implementation)
- Documents assumptions explicitly
- Prevents integration bugs that only appear during testing
- Adds minimal overhead (one verification task per plan)

**Logged as:** ISS-002 in .planning/ISSUES.md
</cross_phase_verification>
