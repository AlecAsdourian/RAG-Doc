...
| 503 | The state store, the App credentials, the App slug, **or the App's client credentials** are unavailable. The flow refuses to start rather than sending the user to GitHub for an installation it could not finish — an abandoned install leaves a live unlinked installation, which is the state the callback's user check exists to protect. |
...
browser following GitHub's redirect sends no bearer token.

Two credentials arrive on it, and both are required:

- **`state`** — single-use, organization-bound, minted by
  `GET /api/github/install`. Says *which workspace* to link to.
- **`code`** — GitHub's user-authorization code, present because the App
  has "Request user authorization (OAuth) during installation" enabled.
  Exchanged for a user-to-server token and used to confirm the caller
  actually has access to the installation they named.

`state` alone is not enough. The app-level installation lookup proves an
installation is real, not that the person completing the callback
controls it — so without the `code` leg, an unlinked installation could be
claimed by any authenticated user who knew its numeric id.
...
| `invalid_installation` | GitHub did not send a usable id, we could not reach that installation, **or the authorizing user does not have access to it**. Deliberately one result for all three. | Offer to start again; suggest checking the App is still installed and that they authorized it. |
...
| `unavailable` | Our GitHub integration or state store is down, or the App's client credentials are not configured. | Retry later; if it persists an administrator needs to finish configuring the App. |
