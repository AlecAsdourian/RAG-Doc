...
**Current focus:** Milestone v1.0 MVP — first shippable version. **Phase 22 is in execution** (pgvector Storage & the First Real Repository). Its plans merged as `ac95151` (PR #47); **22-01 is executed and in review** on `feat/22-01-pgvector-and-seeded-gate`. Scope and decisions are in `.planning/phases/22-repository-clone-ingestion/22-CONTEXT.md`.

## Current Position

Milestone: v1.0 MVP (10 phases: 17-25, plus 22.1 inserted 2026-09-17)
Phase: **22 IN EXECUTION, 2026-09-17: plans 22-01 to 22-05 merged as `ac95151` (PR #47). 22-01 is executed and in review: pgvector everywhere, and ISS-031 fixed behind a seeded-migration gate that failed on the unfixed 000014 and passes after (`22-01-SUMMARY.md`).** Writing the plans moved four boundaries, recorded in `22-CONTEXT.md` under the split. Researched and scoped the same day (PR #45): The user approved splitting it into **Phase 22** (pgvector storage, and one real repository indexed end to end) and **Phase 22.1** (symbols, incremental updates, progress and the code graph), with a retrieval-quality track between 22-03 and Phase 23. The user answered U1–U10 and took every recommendation. **`22-CONTEXT.md` is the authority** for the split, the plans, the answers and every decision; this file keeps no copy of any of them. **Next: review and merge 22-01, then execute 22-02.**
...
Last activity: 2026-09-17. **22-01 executed, in review.**
- **The image.** Every Postgres this project starts is `pgvector/pgvector:pg16`, pinned by digest (`sha256:ccc6e83d…`): compose, both harnesses and CI. Migration 000016 creates the extension, so an image without it fails loudly. The Go harness's reuse container is renamed `rag-doc-isolation-tests-pgv16`.
- **ISS-031 is fixed.** 000014 declares its foreign key inside `CREATE TABLE`, which leaves the schema identical (550 catalog lines match).
- **The gate that proves it** is `TestMigrationsApplyToASeededDatabase`, now in CI. It seeds four tenants at version 10 and migrates in one session as a `NOSUPERUSER NOBYPASSRLS` owner.
  - It failed at 14, dirty, with `22P02` on the commit that added it, and passes after the fix.
...
Previously, 2026-09-17. Phase 22 was researched against the real schema and scoped (PR #45). The user answered U1–U10 the same day. The results are recorded in three places:
...
Progress: v1.0 MVP ████░░░░░░ 4/10 phases complete (17, 19, 20, 21); Phase 22 planned, awaiting approval (Phase 18 Observability deferred — see project_phase18_deprioritized memory)
...
- **ISS-009:** `pkg/vectordb` build — **✅ closed 2026-09-08.** Qdrant client bumped v1.7.0 → v1.19.2; the pin had always predated the API the package was written against. Fixing it surfaced a panicking unit test that had never been able to run. (The package, dead code with no importer, is deleted in 22-01.)
- **ISS-031:** A migration that sets a tenant poisons the migrating session — **HIGH, live in the deployment shape; FIXED for 13 → 14 in 22-01 (in review)** by declaring 000014's foreign key inside `CREATE TABLE`, and the class is now guarded in CI by `TestMigrationsApplyToASeededDatabase`. The rule for every later migration: foreign keys inside `CREATE TABLE`, never rely on the session's tenant, never a sentinel tenant. See ISSUES.md.
...
Stopped at: 22-01 executed and in review on `feat/22-01-pgvector-and-seeded-gate` (`22-01-SUMMARY.md`). The Phase 22 plans merged as `ac95151` (PR #47); research and context as `04b0db0` (PR #45).
Resume file: None

Next command suggested: after 22-01 merges, execute 22-02 (`.planning/phases/22-repository-clone-ingestion/22-02-PLAN.md`). Its 000017 runs through 22-01's seeded gate in one session after 000013 and 000015, and adds its assertions there. **Read `22-CONTEXT.md` first.** It is the authority for plan scope and decisions. Three decisions are still PROPOSED:
- P16, which is provisional in 22-05;
- P8 and P11, which belong to 22.1.

22.1 is planned when Phase 22 is done, from what Phase 22 measures.
