...
Phase: 21 IN PROGRESS — Ingestion Job Infrastructure. 4 of 7 plans executed (21-01 merged as `fbb9793` / PR #37; 21-02 merged as `58ca2e8` / PR #38; 21-03 merged as `3068a42` / PR #39; 21-04 in review); 21-05 … 21-07 not started. Phase 20 COMPLETE — all five plans merged.
...
Last activity: 2026-09-16 — 21-04 executed: the four webhook paths that request or cancel work now do it through `pkg/jobs`. `push` creates an `incremental` job and, when one is already live, JOINS it — replacing the `sync_state <> 'syncing'` guard, which "fixed" ISS-016 by dropping the push. Bulk `installation_repositories.added` runs one lookup with `FOR UPDATE OF r`, decides in Go which installations actually changed, supersedes only those, and then enqueues EVERY matched row in one `jobs.Enqueue` — the L8 case that once queued 1 of 3 while reporting success, now raced against a concurrent relink through a barrier over five rounds. `removed` supersedes the live jobs it stands down. `installation.deleted` supersedes every repository under the installation and drives its stand-down off the ids `SupersedeLive` RETURNED, so a repository whose job was retrying (projected `failed`) cannot be left looking like it is still retrying. Migration 000015 backfills a job for every repository left `pending` by the old path, skipping the unsyncable ones (`installation_id IS NULL`, uninstalled installation), one organization at a time, idempotent by the producer's own `ON CONFLICT` arbiter — and it satisfies ISS-031 by structure: the file ends with the `DO` block, so no DML follows the tenant it sets. The stale comment claiming a failed delivery "is not automatically retried, by design" is corrected: `claimDelivery` re-claims `'failed'`, so every handler runs twice and every producer call is re-entrant.
...
Stopped at: 21-04 executed (`21-04-SUMMARY.md`), PR open for review. 21-01 merged as `fbb9793` (PR #37), 21-02 as `58ca2e8` (PR #38), 21-03 as `3068a42` (PR #39). 21-05 … 21-07 not started.
Resume file: None

Next command suggested: execute 21-05 (`.planning/phases/21-ingestion-job-infrastructure/21-05-PLAN.md`) once 21-04 merges. The plans run in order. 21-05 is the Python consumer and **lifts the claim, completion, failure, sweeper and run-resolution statements verbatim out of `services/backend/pkg/jobs/schema_test.go`**, where 21-02 proved each on PostgreSQL 16, rather than retyping them. Two rules from `pkg/jobs/doc.go` decide most of it: every terminal write is fenced on `id + lease_owner + state = 'running'`, and completion-before-re-enqueue is an ordering rule whose violation raises nothing and loses the rerun. 21-05 also owns the `sync_state` projection for every state after `pending`, which the producers now write and nothing else does.
