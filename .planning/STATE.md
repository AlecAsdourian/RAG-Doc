...
Phase: 21 IN PROGRESS — Ingestion Job Infrastructure. 6 of 7 plans executed (21-01 merged as `fbb9793` / PR #37; 21-02 as `58ca2e8` / PR #38; 21-03 as `3068a42` / PR #39; 21-04 as `0c63917` / PR #40; 21-05 as `a92df5f` / PR #41; 21-06 in review); 21-07 not started. Phase 20 COMPLETE — all five plans merged.
...
Stopped at: 21-05 executed (`21-05-SUMMARY.md`), PR open for review. 21-01 merged as `fbb9793` (PR #37), 21-02 as `58ca2e8` (PR #38), 21-03 as `3068a42` (PR #39), 21-04 as `0c63917` (PR #40). 21-06 and 21-07 not started.
Resume file: None

Next command suggested: execute 21-06 (`.planning/phases/21-ingestion-job-infrastructure/21-06-PLAN.md`) once 21-05 merges. The plans run in order. 21-06 builds the long-running consumer on `workers.jobs`: the loop, a heartbeat on its OWN connection (a shared psycopg2 connection shares its transaction, so a heartbeat commit would commit the handler's work), a periodic sweep, graceful shutdown, and a `python -m workers` that **refuses to start until Phase 22 registers handlers**, so queued jobs are not dead-lettered in the meantime. Its claim-time installation check is what ISS-033's priority depends on: `installation_id IS NULL` or `uninstalled_at` set means `abandon` (superseded, `never_synced`, no walk towards `dead`), and `suspended_at` means `defer` without consuming an attempt. Both transitions already exist and are tested; 21-06 supplies the read that chooses between them. Call `new_worker_id()` ONCE at start, never per job, or the heartbeat's fence disagrees with the claim's.
