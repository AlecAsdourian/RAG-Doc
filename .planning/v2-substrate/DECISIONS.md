...
checks missed, all passing:

| | |
|---|---|
| V1 | the unit-separator escape is a legal separator; the NUL escape fails at PL/pgSQL **compile** time |
| V2 | an archived symbol resurrects via upsert-and-unarchive, not a duplicate-key error |
| V3 | the D5 trigger does not break INSERT (the `TG_OP` guard and UPDATE-only attachment) |
| V4 | cross-org re-parent rejected with a readable error; same-org allowed |
| V5 | **deleting a repository with an anchored memory succeeds** — anchor nulls, digest survives, memory kept |
| V6 | **deleting an organisation completes** — tenant offboarding is not blocked |

V5 and V6 were blockers: `memory_anchors.symbol_id NOT NULL` with a plain FK
made both fail once any memory was anchored, and `DELETE /repositories/{id}` is
a shipped endpoint. **Every schema in this
...
CREATE TABLE chunks (
  ...
  -- (the existing chunks columns carry over unchanged: file_path, start_line,
  --  end_line, content, content_hash, language, chunk_type, breadcrumb,
  --  metadata, created_at, updated_at -- elided here, not omitted)
...
**⚠ And there is no way through, which an earlier revision got backwards.** It
said the escapes were "`SECURITY DEFINER` … or `BYPASSRLS`". `SECURITY DEFINER`
does **not** get through: migration 000012's entire lesson is that
`FORCE ROW LEVEL SECURITY` applies policies to the table owner too, so a definer
function owned by that role is filtered exactly like a direct write. Reproduced
in review — the same `new row violates row-level security policy` either way.
Only `BYPASSRLS` would work, and granting it to escape a constraint we chose is
