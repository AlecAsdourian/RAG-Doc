...
- **SETTLED 2026-09-10, not yet shipped.** Phase 21's context locks the resolution: `.planning/phases/21-ingestion-job-infrastructure/21-CONTEXT.md`, decisions L2 and L4. The fix is **not** a lease on `sync_state`. The root cause is that a *status column* was being used as a *queue*, so Phase 21 introduces `ingestion_jobs` as the work item, demotes `sync_state` to a projection the job writes and the UI reads, and makes a relink **supersede** an in-flight job rather than race it. A partial unique index on `(repository_id) WHERE state IN ('queued','running')` makes two live jobs for one repository unrepresentable, so the guard is in the schema and not only in the code path that remembers it. The `failed`-cannot-retry half gets a state machine with somewhere to go. **Closes when Phase 21 ships.**
