...
provides:
  ...
  - "51 tests: 34 integration against a real PostgreSQL 16 and 17 pure-function unit tests"
...
key-decisions:
  ...
  - "The redact-then-truncate ORDER is not a security guard, and the first version of the comment claiming it was has been corrected. Measured: a truncated token's prefix still matches the pattern, so the other order redacts the fragment too. Mutation X is the deliberate survivor."
...
## `last_error` never carries a credential

`ghs_`, `ghp_`, `github_pat_` and `sk-` (including `sk-proj-`) are replaced
with `[REDACTED]`, and the result is capped at 2,000 characters. The
motivating shape is a failing clone: `last_error` is persisted **and**
returned by 21-07's admin endpoint, and a clone URL is
`https://x-access-token:ghs_…@github.com/…`.

Six unit tests, one per pattern plus multiples, truncation, and a token
straddling the truncation point. Redaction keeps the surrounding context
(`github.com/acme/widgets.git`, `exited 128`) — the point is to keep the
error useful, not to blank it. `defer` and `abandon` put their `reason`
through the same filter.

**One claim in the first cut was wrong and is corrected rather than left
standing.** The comment said redact-before-truncate was a security guard,
because "truncating first can cut a token in half and leave most of it in
the column". **Measured false:** a truncated token's prefix still matches
the pattern, so truncate-then-redact replaces the fragment too. Mutation X
is the deliberate survivor, and what redacting first actually buys —
tidier output length — is now stated as tidiness.
...
**3. `$n` → `%s`, and nothing else.** Every ported statement is otherwise
byte-identical to its Go twin, including the parenthesisation of the
claim's `OR` branches. Two statements gained a `RETURNING` list —
`sweepSQL` (so each dead-lettered job can be logged with its id,
repository and attempts) and `failSQL` (so `fail` reports the state the
database chose rather than re-deriving the `CASE` against a snapshot). Both
additions select no different rows and write nothing extra.
...
assert the property both orders have.
...
|---|---|---|
| Workers, CI's environment | from `services/workers`: `REDIS_URL=redis://localhost:63793/15 OPENAI_API_KEY=sk-test-dummy pytest tests/ workers/ -q`, **no `DATABASE_URL`**, no reachable `.env` (only `.env.example`), fresh venv from `requirements.txt` | **232 passed**, 0 failed, 0 skipped (19 pre-existing `utcnow` deprecation warnings). `main` was 181, so this plan adds 51 |
| This plan's tests alone | `pytest tests/isolation/test_job_transitions.py workers/jobs -q` | **51 passed** — 34 integration, 17 unit |
| Repeated | the same command, 3 consecutive runs | 51 passed each time; no flakes |
| Mutations | 25, on a copy, each proven present in the file before the run | 24 killed, 1 deliberate survivor (X) |
...
and removed afterwards. **The docker-compose Postgres (port 5434) and
...
5. **`abandon` does not decrement `attempts`.** The brief lists "no attempt
   ...
   counter is no longer a budget. The property that matters is tested:
   `the_sweeper_leaves_an_abandoned_job_alone` forces `attempts =
   max_attempts` and the job stays `superseded`.
...
7. **Three tests beyond the plan's list that no mutation demanded**, and
   ...
   the Python side.
8. **The mutation count is 25, not 6.** The extra nineteen follow 21-02's
   and 21-03's practice, and three of them found real gaps.
