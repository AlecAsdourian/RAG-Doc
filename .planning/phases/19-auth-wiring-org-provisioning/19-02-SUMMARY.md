...
> **Retracted premise (added 2026-09-08 during 19-03).** Both entries below reason from "Supabase retries the webhook". It does not — the delivery is once-per-user, and Phase 4 recorded as much in `04-06-SUMMARY.md:282`. Both *fixes* remain correct and are unchanged in the code; only the justifications written here were wrong, and 19-03 inherited that wrong model into a blocker. See `19-03-SUMMARY.md` → "The blocker review caught".

**H3 — a user could end up permanently organization-less**
- **Was:** org creation was gated on `isNewUser`. If it failed after the user row committed, the branch was skipped on any later run and 202 was returned. The user had no org, forever. *(The original text said "500 → Supabase retries → …". There are no retries; the hazard was real regardless, since the flag made the handler non-idempotent for any replay, manual or automated.)*
- **Now:** gated on actual ownership via a new `UserHasOwnerOrg`. The handler is idempotent — however many times it runs, the end state is one user with one owner org. Pinned by `TestProvisioningIsolation_OrgCreationRecoversAfterFailure`. What makes this *reachable* rather than theoretical is `cmd/backfill-org-claims`, shipped in 19-03.
...
- **Now:** the post-conflict SELECT compares `user.SupabaseUserID` against the incoming id and returns `ErrEmailOwnedByAnotherIdentity` on mismatch. The webhook maps it to **409 Conflict** rather than 5xx, because this is a permanent condition needing manual reconciliation and the status is what a human reading the delivery log sees. *(The original text justified 409 as "so Supabase stops retrying". Wrong reason, right status.)* Pinned by `TestProvisioningIsolation_EmailOwnedByAnotherIdentityIsRefused`.
